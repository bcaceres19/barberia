...
| B3 · Agenda, estados e integridad | `HU-060` – `HU-065` | `HU-060`–`HU-062` integradas; `HU-063` implementada (issue [#116](https://github.com/bcaceres19/barberia/issues/116)), abierta como [PR #118](https://github.com/bcaceres19/barberia/pull/118) pendiente de CI; `HU-064` propuesta con issue pendiente; `HU-065` además bloqueada por `DP-CIT-06` |
...
> **B3 en ejecución.** Las primeras tres historias separaron la última defensa de datos (`HU-060`), la primera escritura vertical de negocio (`HU-061`) y la pantalla operativa mínima (`HU-062`); las tres están integradas. El segundo lote amplía esa base en cortes independientes: navegación por fecha (`HU-063`), detalle con historial (`HU-064`) y reprogramación auditada T2 (`HU-065`). `HU-063` tiene issue real ([#116](https://github.com/bcaceres19/barberia/issues/116)) y está implementada, abierta como [PR #118](https://github.com/bcaceres19/barberia/pull/118) pendiente de CI. `HU-064`/`HU-065` permanecen `draft` con `issue: pending`; la redacción no autoriza crear ramas de implementación ni combinar las tres entregas. `HU-065` conserva además el bloqueo explícito `DP-CIT-06`.
...
| Estado | Implementada contra el issue real [#116](https://github.com/bcaceres19/barberia/issues/116) en `feat/116-hu063-navegacion-agenda`, abierta como [PR #118](https://github.com/bcaceres19/barberia/pull/118) pendiente de CI; prompt `PROMPT-HU-063-v1` `in_progress` |
