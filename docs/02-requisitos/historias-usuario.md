...
| B3 · Agenda, estados e integridad | `HU-060` – `HU-062` | `HU-060` integrada (issue real [#104](https://github.com/bcaceres19/barberia/issues/104), PR pendiente de abrir); `HU-061`/`HU-062` propuestas, prompts `draft`; `DP-CIT-01`–`DP-CIT-05` abiertas |
...
> **B3 en ejecución.** Las primeras tres historias separan la última defensa de datos (`HU-060`), la primera escritura vertical de negocio (`HU-061`) y la pantalla operativa mínima (`HU-062`). `HU-060` está integrada (issue real [#104](https://github.com/bcaceres19/barberia/issues/104), PR pendiente de abrir contra `main`): no inventa una API ni una interfaz, solo la base persistente y la primitiva transaccional interna. `HU-061` no puede pasar a `ready` hasta resolver `DP-CIT-01`–`DP-CIT-03`; `HU-062` depende además de `DP-CIT-04` y `DP-CIT-05`. Ambas requieren issue real propio.
...
| Estado | Integrada en `main` (rama `feat/104-hu060-nucleo-citas`, issue real [#104](https://github.com/bcaceres19/barberia/issues/104), PR pendiente de abrir); prompt `PROMPT-HU-060-v1` `executed` |
