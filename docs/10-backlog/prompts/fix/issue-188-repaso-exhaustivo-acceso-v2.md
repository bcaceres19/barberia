...
```text
El PASS anterior de `/acceso` queda revocado. Lee y ejecuta completo `docs/10-backlog/prompts/fix/issue-188-repaso-exhaustivo-acceso-v2.md` sobre la rama y PR actuales. No corrijas solo el asterisco: reinventa y verifica todos los elementos visibles contra los paneles exactos de 1440 y 360 px, elimina los `*` visuales conservando `required`/`aria-required` y validación, resuelve el locator ambiguo de contraseña, genera y abre evidencia v2 baseline/final/lado-a-lado/overlay y autocorrige hasta PASS sin pedirme que encuentre más diferencias.
```
