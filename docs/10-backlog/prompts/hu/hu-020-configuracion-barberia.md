...
## Ejecución real (2026-08-23)

Entrega vertical completa contra el issue [#67](https://github.com/bcaceres19/barberia/issues/67), rama `feat/67-hu020-configuracion-barberia`. Preflight verificado explícitamente: `origin/main` en `a57354b`, CI en verde (job `Go`: `hu001`/`hu005`/`hu007`/`hu008` suites + `go test -race`; job `database`: `atlas migrate validate/apply` + suites RLS/idempotencia; job `web`: lint/typecheck/test:unit/build), documentado en el PR `docs(control)` que preparó los issues (ver historial de cambios).

| Criterio | Estado | Prueba o evidencia |
| --- | --- | --- |
| `CA-020-01` | Cumplido | `GET /private/settings/barbershop` devuelve exactamente los cuatro campos autorizados, aislado por tenant (RLS + filtro explícito). `TestGetBarbershopSettingsHandler_Success_ReturnsFourAuthorizedFields`, `TestGet_ReturnsTheFourAuthorizedFields` (PostgreSQL real), `TestSettings_HTTP_ValidSession_ReturnsOwnBarbershop` (router de producción, PostgreSQL real). |
| `CA-020-02` | Cumplido | `PATCH` persiste y la cabecera se actualiza sin recargar: `TestService_Update_Success_ReturnsSavedRepresentation`, `TestUpdate_ValidContact_PersistsAndRoundTrips` (PostgreSQL real), `TestSettings_HTTP_UpdateThenGet_PersistsAndRoundTrips`, `SettingsPage.test.ts` ("on success, updates the header via updateBarbershopName"), y E2E real `e2e/configuracion-barberia.spec.ts` ("editar y guardar actualiza la cabecera de inmediato y persiste tras recargar"). |
| `CA-020-03` | Cumplido | Zona no reconocida responde `422` con cero escritura (nombre/contacto anteriores intactos): `TestService_Update_RepositoryReportsInvalidTimezone_ReturnsValidationNoWrite`, `TestUpdate_InvalidTimezone_WritesNothing` (PostgreSQL real, 4 formas inválidas), `TestSettings_HTTP_InvalidTimezone_Returns422WithoutPartialWrite`, y E2E real ("una zona horaria no reconocida se rechaza sin perder los demás datos escritos"). |
| `CA-020-04` | Cumplido | `formatInstantInTimezone` (`apps/web/src/shared/time/formatInstant.ts`) recibe la zona siempre explícita, nunca la del dispositivo; probado con el mismo instante en `America/Bogota` y `Europe/Madrid` (resultados distintos) y con control negativo contra la zona del entorno de pruebas (`formatInstant.test.ts`). Sin consumidor de UI todavía: esta pantalla no formatea ningún instante; queda lista para B2/B3. |
| `CA-020-05` | Cumplido | `barbershopId` no existe como campo del contrato; un intento en el cuerpo se rechaza `400` (forma inválida), nunca se interpreta como tenant: `TestUpdateBarbershopSettingsHandler_UnknownField_ReturnsInvalidRequest`, `TestSettings_HTTP_UnknownField_Returns400`, `TestContract_UpdateBarbershopSettingsRequestSchema_MatchesDTOFields` (confirma que el schema no declara ese campo). Aislamiento cruzado con dos tenants reales: `TestGetAndUpdate_TenantAIsolatedFromTenantB`, `TestSettings_HTTP_TwoTenants_UpdateNeverCrossesBarbershops`, más `database/tests/hu020_configuracion_barberia.sql` (RLS con SQL directo). |
| `CA-020-06` | Cumplido | Contacto vacío se normaliza a `NULL` (nunca cadena vacía), correo a minúsculas, ambos validados sin registrar su valor: `TestService_Update_EmptyContacts_NormalizeToNilBeforeCallingRepository`, `TestService_Update_ContactEmail_NormalizesToLowercaseAndTrimmed`, `TestUpdate_NilContact_PersistsAsNullNotEmptyString` (PostgreSQL real), `TestSettings_HTTP_ThroughFullRouter_NeverLogsContactInfo` (router de producción, log inspeccionado). |
| `CA-020-07` | Cumplido | Contrato, handler, cliente generado y formulario coinciden en los cuatro campos: `TestContract_BarbershopSettingsResponseSchema_MatchesDTOFields`, `TestContract_UpdateBarbershopSettingsRequestSchema_MatchesDTOFields`, `TestContract_GetBarbershopSettingsOperation_*`, `TestContract_UpdateBarbershopSettingsOperation_*` (contra `api/openapi/paths/settings.yaml` fuente), cliente TypeScript regenerado desde el bundle lintado. |
| `CA-020-08` | Cumplido | Estados discriminados (carga/listo/guardando/error de campo/error recuperable/éxito); un error recuperable conserva lo escrito: `SettingsPage.test.ts` ("on a recoverable network error, keeps the typed values...", "on a validation-error..., keeps the typed values..."), doble envío bloqueado, y evidencia visual/teclado en 320/360/768/1280 px + zoom 200 % (`e2e/configuracion-barberia-evidencia-responsiva.spec.ts`, capturas inspeccionadas en `e2e/evidence/configuracion-barberia/`), `vitest-axe` sin violaciones en `SettingsPage.test.ts`. |

Exclusiones verificadas: sin aprovisionamiento/selección de barbería, sin `public_slug`/logo/reserva pública, sin barberos/servicios/horarios/agenda, sin tema/paleta por barbería, sin reescritura de instantes existentes, sin catálogo de zonas nuevo ni selector basado en el navegador, sin estado global paralelo (`sessionStore` singleton existente reutilizado), sin segundo cascarón ni segundo guard.
