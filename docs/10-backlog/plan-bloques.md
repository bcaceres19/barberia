...
| **B3** | Agenda del barbero, estados e integridad | 11 | `HU-060` – `HU-0xx` | `HU-060`–`HU-062` integradas; `HU-063` navegación por fecha implementada (issue real [#116](https://github.com/bcaceres19/barberia/issues/116)), pendiente de PR; `HU-064` detalle/historial y `HU-065` reprogramación auditada redactadas como propuesta; `HU-065` bloqueada por `DP-CIT-06` |
