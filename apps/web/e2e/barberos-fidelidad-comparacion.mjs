/*
 * Comparación de evidencia del issue #192. El atlas reúne escritorio y móvil
 * en una lámina, por eso se recortan los paneles de Barberos antes de
 * componer referencia | app y su diferencia absoluta. No es una suite.
 *
 *   node apps/web/e2e/barberos-fidelidad-comparacion.mjs
 */
...
const captures = resolve(here, 'evidence/barberos/fidelidad-192')
const output = resolve(captures, 'comparacion')
const dataUri = (file) => `data:image/png;base64,${readFileSync(file).toString('base64')}`

const panels = [
  // Los rectángulos están medidos en la lámina original 1536×1024.
  { name: 'desktop', crop: [8, 10, 742, 514], actual: 'desktop/lista.png' },
  { name: 'mobile', crop: [994, 552, 248, 440], actual: 'mobile/lista.png' },
]
...
async function main() {
  ...
  for (const panel of panels) {
    ...
    save('lista-lado-a-lado.png', result.side)
    save('lista-diff.png', result.diff)
    report.push(
