...
| `idempotency_concurrency.sql` | Protocolo de idempotencia (`idempotency_begin`/`complete`/`abort`/`purge_expired`, DEC-043, HU-004): proceed/replay/conflict_operation/conflict_fingerprint, abort nunca borra `completed` (`DDL-IDEM-01`), aislamiento tenant-aware, vencimiento, límite exclusivo de `barberia_worker`; secuencial, una sola conexión |
| `idempotency_concurrency_two_connections.sh` | La misma clave, pero con dos conexiones `psql` reales y solapadas, para probar que `pg_try_advisory_xact_lock` responde `locked` sin espera mientras la transacción de la otra sigue abierta (CA-004-03); requiere `DATABASE_TEST_URL` y `psql` en el `PATH` |
