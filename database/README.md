...
| `migrations/20260827110000_create_appointment_core.sql` | HU-060 · `customer` (persona que reserva, `phone`/`email` opcionales por RN-CIT-02, únicos por barbería según `DEC-045`/`DEC-046`), `appointment` (cinco estados cerrados, columna generada `occupies_schedule`, snapshots de servicio por `DEC-004`, restricción `EXCLUDE USING gist (barbershop_id, barber_id, tstzrange(starts_at, ends_at, '[)')) WHERE (occupies_schedule)` como última defensa contra cruces del mismo barbero), `appointment_history`/`appointment_history_change` (append-only, vocabulario cerrado de `event_type` por `DEC-041`), basadas en la sección D.0–D.2 de `modelo-fisico-referencia.sql` sin diferencias de fondo con el esquema real ya migrado. FK tenant-aware en `ON DELETE RESTRICT` (`DEC-070`) hacia `barber`/`service`/`customer`/`staff_user`. RLS forzada; `customer`/`appointment` con `SELECT`/`INSERT`/`UPDATE` sin `DELETE` (RN-DAT-03); `appointment_history`/`appointment_history_change` con `SELECT`/`INSERT` únicamente (sin `UPDATE` ni `DELETE`, RN-HIS-02). Ningún endpoint HTTP ni contrato OpenAPI: solo la primitiva transaccional interna de `internal/modules/booking` |
...
| `migrations/20260919200000_add_auth_recovery_resolve_phone.sql` | HU-008 · `auth_recovery_resolve_phone(p_phone text) RETURNS text`, función `SECURITY DEFINER` estrecha que resuelve un número E.164 verificado al correo de la única cuenta activa que lo tiene, más el índice parcial `idx_staff_user_verified_phone`; devuelve `NULL` con cero o con más de una coincidencia sin distinguirlas (`DEC-092`, `DEC-093`, `DP-SEG-14`, `DEC-065`) |
...
| `testdata/hu008_recuperacion_canal.sql` | HU-008 · teléfonos verificados ficticios sobre cuentas que ninguna otra suite de recuperación usa: `dueno.o` con un número único (recorrido por WhatsApp, con credencial ficticia) y `dueno.p`/`dueno.q` con el MISMO número en barberías distintas (ambigüedad, `DP-SEG-14`); requiere `hu042_bloqueos.sql` y `hu060_citas.sql` cargados antes |
...
| `tests/hu060_citas.sql` | HU-060 · esquema de `customer`/`appointment`/`appointment_history`/`appointment_history_change`, `CHECK` de forma (nombre/teléfono/correo/anonimización de `customer`; estado/origen/intervalo/snapshots/duración-coherente-con-intervalo/motivo-de-cancelación/`resolved_at` de `appointment`; vocabulario/forma-de-actor/motivo de `appointment_history`), la restricción `EXCLUDE USING gist` rechazando cruce total/parcial por ambos extremos/de un minuto y aceptando contigüidad/otro barbero/tenant distinto/cita cancelada solapada (`CA-060-02`), `occupies_schedule` correcto para los cinco estados incluida una cita `completed` que sigue bloqueando la exclusión (`CA-060-03`), duración coherente cruzando medianoche y en una fecha de cambio de horario de verano con la sesión en otra zona (`CA-060-04`), historial append-only (`INSERT` permitido, `UPDATE`/`DELETE` rechazados con el rol real), RLS forzada/sin `BYPASSRLS`, grants exactos, aislamiento de tenant con id real (`RN-TEN-01`) y `ON DELETE RESTRICT` verificado contra PostgreSQL real hacia `barber`/`service`/`customer`/`staff_user`/`appointment`/`appointment_history` (`DEC-070`); la primitiva transaccional completa (cliente+cita+historial en una sola `InTenantTx`, incluida la carrera real de dos conexiones) vive en `internal/modules/booking/postgres`, todo con el rol real |
