...
-- datos, no una llamada de red, así que puede ejecutarse dentro de la MISMA
-- transacción que hace el claim. El `FOR UPDATE OF c SKIP LOCKED` ya
-- mantiene la fila bloqueada hasta que esa transacción confirma; no hace
-- falta un lease con claim_token porque no hay una ventana externa que
-- proteger. Si la anonimización completa (issue de privacidad, DEC-049)
-- termina partiéndose en varias transacciones, esta decisión debe
...
COMMENT ON FUNCTION retention_claim_due_customers(integer, timestamptz) IS
  'Reclama clientes con datos personales vencidos (p_limit 1-200, DDL-OPS-01). Devuelve SOLO '
  'identificadores. La anonimización efectiva la ejecuta el servicio, dentro del contexto de '
  'tenant y de la MISMA transacción que reclamó la fila (DEC-053: sin lease, a diferencia de '
  'notification_claim_due, porque no hay E/S externa entre reclamar y escribir), y debe: '
